...
// Backend (Node SDK) uses camelCase; tolerate snake_case too.
const ts = (o) => o.createdAt || o.created_at;

// `status` is a rich object {state, terminationType, label} on Call objects,
// or a plain string on live call.ended events. Normalise either way.
...
function statusClass(s) {
  ...
  if (["failed", "canceled", "cancelled", "no-answer", "no_answer", "busy"].includes(t)) return "failed";
...
function isTerminal(s) {
  ...
  return ["completed", "failed", "canceled", "cancelled", "no-answer", "no_answer", "busy"]
...
$("#smsForm").addEventListener("submit", async (e) => {
  ...
  btn.disabled = true;
  try {
    const data = await postJSON("/api/sms", { to: f.to.value, body: f.body.value });
...
function renderEvent(ev, prepend = true) {
  ...
  el.className = cls;
  el.innerHTML = `<div class="ev-top"><span class="ev-type">${esc(title)}</span><span>${fmtTime(ts(ev))}</span></div>${inner}`;
...
async function loadNumbers() {
  try {
    const { numbers, defaultNumberId } = await (await fetch("/api/numbers")).json();
    const def = numbers.find((n) => n.id === defaultNumberId) || numbers[0];
    if (def) {
      $("#myNumber").textContent = def.number;
      $("#myCaps").textContent = def.capabilities;
      const fn = $("#feedNumber"); if (fn) fn.textContent = def.number;
    } else {
      $("#myNumber").textContent = "No number provisioned";
      $("#myNumber").classList.add("warn");
    }
  } catch (_) {
  ...
  try {
    ...
    calls.forEach((c) => {
      ...
      row.innerHTML = `
        ...
        <span><strong>${esc(other)}</strong><br><span class="ev-meta">${fmtTime(ts(c))}</span></span>
  ...
  try {
    ...
    body.innerHTML = `
      ...
      <div class="kv">
        ...
        <span>Created</span><span>${fmtTime(ts(call))}</span>
      </div>
